...
async def body_tracking_loop(vts_api):
    ...
    while True:
        try:
            if not vts_api._ws: await asyncio.sleep(1); continue

            hwnd = win32gui.FindWindow(None, "\u200b")

            if not hwnd: hwnd = win32gui.FindWindow(None, "VTube Studio")
            if not hwnd: await asyncio.sleep(1); continue
            ...
            # Eje X
            if dx < 0: ratio_x = dx / center_x if center_x > 0 else 0
            else: dist_right = screen_width - center_x; ratio_x = dx / dist_right if dist_right > 0 else 0
            ...
            # Eje Y
            if dy < 0: ratio_y = -dy / center_y if center_y > 0 else 0
            else: dist_bottom = screen_height - center_y; ratio_y = -dy / dist_bottom if dist_bottom > 0 else 0
