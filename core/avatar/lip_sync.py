...
def start_lip_sync(vts_api, text, duration, loop): 
    asyncio.run_coroutine_threadsafe(lip_sync_sim(vts_api, text, duration), loop)
...
async def lip_sync_sim(vts_api, text, duration):
    ...
    events = []
    for c in text.lower():
        if c in VOWELS or c == 's': events.append(('vowel', c))
        elif c == ' ': events.append(('space', ' '))
        elif c in PUNCTUATION_SHORT: events.append(('pause_short', c))
        elif c in PUNCTUATION_LONG: events.append(('pause_long', c))
    ...
    try:
        for kind, char in events:
            ...
            if kind == 'vowel':
                ...
                current = await _lerp_to(vts_api, target[0], target[1], current, steps=4, interval=slot * 0.6 / 4)
                current = await _lerp_to(vts_api, REST[0], REST[1], current, steps=3, interval=slot * 0.4 / 3)

            elif kind == 'space': 
                current = await _lerp_to(vts_api, REST[0], REST[1], current, steps=2, interval=slot / 2)

            elif kind == 'pause_short': 
                current = await _lerp_to(vts_api, PAUSE_SHORT[0], PAUSE_SHORT[1], current, steps=3, interval=slot / 3)

            elif kind == 'pause_long': 
                current = await _lerp_to(vts_api, PAUSE_LONG[0], PAUSE_LONG[1], current, steps=4, interval=slot / 4)
