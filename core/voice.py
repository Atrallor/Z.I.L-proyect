...
def speak_text(text: str, avatar_api=None, loop=None) -> None:
    if not CONFIG["use_tts"] or not text: return
    ...
    def _tts_thread():
        ...
        try:
            if _tts_interrupted.is_set(): return
            ...
            async def generate():
                communicate = edge_tts.Communicate(text,"es-CO-SalomeNeural",rate="+15%",pitch="+12Hz",)
            ...
            if avatar_api and loop: start_lip_sync(avatar_api, text, duration, loop)
            ...
            with _alias_lock: _current_alias = alias
            ...
            while True:
                if _tts_interrupted.is_set():
                    ctypes.windll.winmm.mciSendStringW(f"stop {alias}", None, 0, 0)
                    break
                ctypes.windll.winmm.mciSendStringW(f"status {alias} mode", status_buf, 128, 0)
                if status_buf.value != "playing": break
                time.sleep(0.05)

            ctypes.windll.winmm.mciSendStringW(f"close {alias}", None, 0, 0)
            tts_playing.clear()                       
            with _alias_lock: _current_alias = None
            ...
            tts_playing.clear()
            tts_busy.clear()
            if lock_held: _tts_lock.release()
